...
def add_trace(fig, G, city_1, city_2):
    ...
    if city_1_lon < 0 < city_2_lon and np.abs(city_1_lon) > 90 and np.abs(city_2_lon) > 90:
        city_1_lon = city_1_lon + 360
    elif city_1_lon > 0 > city_2_lon and np.abs(city_1_lon) > 90 and np.abs(city_2_lon) > 90:
...
def plot_cities(cities_graph, us_cities, ga_obj):
    ...
    fig = add_trace(fig, cities_graph, ga_obj.best_individual_[0], ga_obj.best_individual_[-1])
